...
from src.prototype.dia_scale.constants import COLOR_FLOOR_BG, COLOR_FLOOR_FG
...
from src.render.iso_character_view import EDITOR_FLOOR_TILES, PITCH_CHARACTER_DEG, YAW_DEFAULT_DEG, ZOOM_FIXED
...
class IsoCharacterRenderer:
    ...
    def _draw_floor_grid(
        ...
        *,
        ...
        half = EDITOR_FLOOR_TILES // 2
        sx, sy = ISO_STEP_X, ISO_STEP_Y
        for dx in range(-half, half + 1):
            for dy in range(-half, half + 1):
                wx = cx0 + dx
                wy = cy0 + dy
                _, su, sv = self._project_point(wx, wy, floor_z, cx0=cx0, cy0=cy0, cz0=cz0, orbit=orbit, zoom=zoom)
                px = ox + su * cw
                py = oy + sv * ch
                tip = (px, py)
                verts = [
                    tip,
                    (tip[0] + sx * cw, tip[1] + sy * ch),
                    (tip[0], tip[1] + 2 * sy * ch),
                    (tip[0] - sx * cw, tip[1] + sy * ch),
                ]
                shaded = (dx, dy) in floor_shadow or (int(wx), int(wy)) in floor_shadow
                fill = tuple(max(0, c - 18) for c in COLOR_FLOOR_BG) if shaded else COLOR_FLOOR_BG
                fg = tuple(max(0, c - 25) for c in COLOR_FLOOR_FG) if shaded else COLOR_FLOOR_FG
                pygame.draw.polygon(surf, fill, verts)
                pygame.draw.polygon(surf, fg, verts, 1)
        ...
        if projected_raw or show_floor:
            ...
            if show_floor:
                self._draw_floor_grid(
                    surf, floor_z=floor_z, cx0=cx0, cy0=cy0, cz0=cz0,
                    orbit=orbit, zoom=zoom, ox=ox, oy=oy, cw=cw, ch=ch,
