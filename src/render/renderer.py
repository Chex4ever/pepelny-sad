...
class AsciiRenderer:
    ...
    def _draw_cells(self, target, buffer, global_alpha: int = 255, *, fill_bg: bool = True):
        ...
        for y in range(h):
            for x in range(w):
                ...
                if ch == " " and buffer.fog[i] == 0 and bg_a < 8:
                    continue
                if ch == " " and buffer.fog[i] == 0 and bg_a >= 250 and fill_bg:
                    continue

                if bg_a > 0:
                    self._cell.fill((*bg, min(255, int(bg_a * global_alpha / 255))))
                    target.blit(self._cell, (px, py))

                if ch != " ":
                    fg = self._tint(buffer.fg[i], light)
                    fg_a = min(255, int(buffer.fg_a[i] * global_alpha / 255))
                    if fg_a > 0:
                        target.blit(self._glyph(ch, fg, fg_a), (px, py))
                ...
                if fog != 0:
                    ...
                    target.blit(self._overlay, (px, py))

    def draw_to_surface(self, surface, buffer, global_alpha: int = 255):
        self._draw_cells(surface, buffer, global_alpha, fill_bg=False)

    def draw(self, buffer, global_alpha: int = 255):
        self._draw_cells(self.screen, buffer, global_alpha, fill_bg=True)
        pygame.display.flip()
