...
class PauseWindow(ModalWindow):
    ...
    def __init__(self):
        w, h = 44, 14
        super().__init__(t("ui.pause.title"), x=(SCREEN_W - w) // 2, y=(SCREEN_H - h) // 2, w=w, h=h, visible=False)
        self.cursor = 0
        self.confirm_quit = False

    def _label(self, key: str) -> str:
        return t(f"ui.pause.{key}")
    ...
    def draw(self, buf: ScreenBuffer) -> None:
        ...
        self.title = t("ui.pause.confirm_title") if self.confirm_quit else t("ui.pause.title")
        super().draw(buf)
    ...
    def draw_content(self, buf: ScreenBuffer) -> None:
        ...
        if self.confirm_quit:
            buf.draw_text(cx + 3, cy + 2, t("ui.pause.confirm_quit"), fg=COLOR_TEXT, bg=COLOR_UI_BG)
            buf.draw_text(cx + 3, cy + 4, t("ui.pause.confirm_yes"), fg=COLOR_TEXT, bg=COLOR_UI_BG)
            return
        for i, key in enumerate(self.OPTIONS):
            fg = COLOR_HIGHLIGHT if i == self.cursor else COLOR_TEXT
            buf.draw_text(cx + 3, cy + 1 + i, self._label(key), fg=fg, bg=COLOR_UI_BG)
        buf.draw_text(cx + 3, cy + ch - 1, t("ui.pause.close"), fg=COLOR_TEXT, bg=COLOR_UI_BG)
