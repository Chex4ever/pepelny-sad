...
def _tree_suppressed(wx: int, wy: int, tree_mask: set[tuple[int, int]]) -> bool:
    return (wx, wy) in tree_mask


def _mark_tree_area(
    wx: int, wy: int, tree_mask: set[tuple[int, int]], radius: int = 2
) -> None:
    for dx in range(-radius, radius + 1):
        for dy in range(-radius, radius + 1):
            tree_mask.add((wx + dx, wy + dy))


def sample_surface_into_chunk(
    chunk: Chunk,
    lx: int,
    ly: int,
    wx: int,
    ...
    if tree_chance > 0 and not _tree_suppressed(wx, wy, tree_mask) and rng.random() < tree_chance:
        place_tree_anchor(chunk, wx, wy, biome, seed, world_map=world_map)
        _mark_tree_area(wx, wy, tree_mask)
...
def sample_surface(world_map, wx: int, wy: int, seed: int, rng: random.Random) -> None:
    ...
    sample_surface_into_chunk(
        chunk, lx, ly, wx, wy, seed, rng, _tree_mask, world_map
    )
...
def _fill_chunk(
    ...
    for ly in range(CHUNK_SIZE):
        for lx in range(CHUNK_SIZE):
            ...
            wy = cy * CHUNK_SIZE + ly
            sample_surface_into_chunk(
                chunk, lx, ly, wx, wy, seed, rng, tree_mask, world_map
            )
...
def generate_chunk(cx: int, cy: int, seed: int, world_map=None) -> Chunk:
    ...
    return _fill_chunk(chunk, cx, cy, seed, _tree_mask, world_map)


def generate_chunk_isolated(cx: int, cy: int, seed: int) -> Chunk:
    """Worker-process chunk (trees baked on main thread via apply_tree_solids)."""
    return _fill_chunk(Chunk(cx, cy), cx, cy, seed, set(), world_map=None)
