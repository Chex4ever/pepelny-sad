...
- **Отрисовка** в редакторе: `floor_iso_draw_map()` — span-fill по строкам iso-экрана, без дыр (`test_editor_floor_iso.py::test_floor_iso_screen_draw_has_no_row_gaps`). Это **не** тот же тест, что ASCII-эталон раскладки.
